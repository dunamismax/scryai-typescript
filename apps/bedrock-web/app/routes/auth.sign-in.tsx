import { APIError } from "better-auth/api";
import { data, Form, redirect, useActionData, useNavigation } from "react-router";
import { z } from "zod";
import { Button } from "~/components/ui/button";
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from "~/components/ui/card";
import { Input } from "~/components/ui/input";
import { Label } from "~/components/ui/label";
import { auth } from "~/lib/auth.server";
import { extractSetCookieHeader, getAuthState, safeRedirectPath } from "~/lib/auth-utils.server";
import { assertSameOrigin } from "~/lib/http.server";
import { checkRateLimit, requestFingerprint } from "~/lib/rate-limit.server";
import type { Route } from "./+types/auth.sign-in";

const signInSchema = z.object({
  email: z.email().max(320),
  password: z.string().min(12).max(128),
});

export async function loader({ request }: Route.LoaderArgs) {
  const authState = await getAuthState(request);
  if (authState) {
    return redirect("/dashboard");
  }
  const url = new URL(request.url);
  return data({ next: safeRedirectPath(url.searchParams.get("next")) });
}

export async function action({ request }: Route.ActionArgs) {
  assertSameOrigin(request);

  const fingerprint = requestFingerprint(request);
  if (!checkRateLimit(`signin:${fingerprint}`, 10, 60_000)) {
    return data({ error: "Too many sign-in attempts. Try again in a minute." }, { status: 429 });
  }

  const formData = await request.formData();
  const next = safeRedirectPath(formData.get("next"));

  const parsed = signInSchema.safeParse({
    email: formData.get("email"),
    password: formData.get("password"),
  });

  if (!parsed.success) {
    return data({ error: "Enter a valid email and password." }, { status: 400 });
  }

  try {
    const result = await auth.api.signInEmail({
      headers: request.headers,
      body: {
        email: parsed.data.email,
        password: parsed.data.password,
      },
      returnHeaders: true,
    });

    const setCookie = extractSetCookieHeader(result);

    return redirect(next, {
      headers: setCookie ? { "set-cookie": setCookie } : undefined,
    });
  } catch (error) {
    if (error instanceof APIError) {
      return data({ error: error.message }, { status: error.statusCode || 400 });
    }
    throw error;
  }
}

export default function SignIn({ loaderData }: Route.ComponentProps) {
  const actionData = useActionData<typeof action>();
  const navigation = useNavigation();
  const pending = navigation.state === "submitting";

  return (
    <main className="mx-auto max-w-md px-4 py-14 sm:px-0">
      <Card>
        <CardHeader>
          <CardTitle>Sign in</CardTitle>
          <CardDescription>Use your account to access dashboard and admin routes.</CardDescription>
        </CardHeader>
        <CardContent>
          <Form className="space-y-4" method="post">
            <input type="hidden" name="next" value={loaderData.next} />
            <div className="space-y-2">
              <Label htmlFor="email">Email</Label>
              <Input autoComplete="email" id="email" name="email" required type="email" />
            </div>
            <div className="space-y-2">
              <Label htmlFor="password">Password</Label>
              <Input
                autoComplete="current-password"
                id="password"
                name="password"
                required
                type="password"
              />
            </div>
            {actionData?.error ? <p className="text-sm text-red-600">{actionData.error}</p> : null}
            <Button className="w-full" disabled={pending} type="submit">
              {pending ? "Signing in..." : "Sign in"}
            </Button>
          </Form>
        </CardContent>
      </Card>
    </main>
  );
}
